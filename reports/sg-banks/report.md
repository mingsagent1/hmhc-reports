...
5. **[±] Monetization score vs benchmark peers** — In line with the index bank, not ahead of it: the SG banks monetize their deposit base at 85–98 vs HSBC = 100, while the US franchises monetize far harder. On the capital-base lens the order reshuffles because the denominator adds each bank's self-defined wealth AUM — pure wealth balance sheets crash the ratio (UBS 43), narrow AUM definitions inflate it (UOB 106) — which is why the two indices are read together.

   | Bank | vDep | vCap | Top Other-Revenue (% of rev) |
   |---|---:|---:|---|
   | DBS | 98 | 100 | fees 21% · trading 15% |
   | OCBC | 89 | 91 | fees 17% · trading 12% · insurance 7% |
   | UOB | 85 | 106 | fees 19% · other non-II 14% |
   | HSBC | 100 | 100 | trading 29% · fees 20% · insurance 5% |
   | UBS | 165 | 43 | fees 56% · trading 28% |
   | JPMorgan | 187 | 119 | AM fees 11% · IB fees 6% · cards 3% |
   | BofA | 147 | 80 | AM fees 14% · IB fees 6% · service 6% |
   | StanChart | 103 | 103 | trading & other 51% · fees 20% |
   | CMB | 90 | 60 | fees 22% · other non-II 14% |
   | CBA | 79 | n/r | other op income 15% |

   *Indexed to HSBC = 100, latest full FY. vDep = `Monetization_vDeposits` (total revenue ÷ customer deposits) · vCap = `Monetization_vCapitalBase` (total revenue ÷ (deposits + wealth AUM)); both within-bank, so currencies cancel. SG raw values (FY25): vDep 3.75% · 3.41% · 3.24%; vCap 2.09% · 1.89% · 2.20%. Category labels compressed here — full wording in Table 6. Peer data fetched 2026-07-24 (independent retriever, `PxClOpus4.8`); indices computed deterministically by `code/build_benchmarks.py`. Sources: Table 6; Tables 1; Appendix B.*

6. **[–] Relative valuation premium vs benchmark peers, and the 5-year growth outperformance required** — The premium is real and, for DBS and OCBC, demanding: on the primary-driver lens P/CapitalBase they'd need **+11.9%** and **+9.4% per year** of outperformance for five years to justify their multiples, with P/Rev saying the same and only the earnings lens kinder. UOB is priced near parity on every index; among peers only Commonwealth Bank is richer, and this peer premium stacks on top of the banks' own history (2.96 / 2.14 / 1.45 P/B, 96% / 84% / 24% above their 10-yr averages).
   ...
   | HSBC | mcap 23 Jul | 100 | +0.0% | 100 | +0.0% | 100 | +0.0% | 100 | +0.0% |
   | UBS | mcap 23 Jul | 27 | −22.9% | 63 | −8.7% | 125 | +4.6% | 101 | +0.2% |
   | JPMorgan | mcap 22 Jul | 119 | +3.5% | 99 | −0.1% | 98 | −0.3% | 145 | +7.8% |
   | BofA | mcap 20 Jul | 59 | −10.1% | 73 | −6.0% | 84 | −3.4% | 80 | −4.5% |
   | StanChart | mcap 3 Jul | 56 | −11.0% | 54 | −11.5% | 69 | −7.1% | 71 | −6.7% |
   | CMB | mcap 6 Jul | 33 | −19.7% | 55 | −11.2% | 38 | −17.4% | 43 | −15.7% |
   | CBA | mcap Jul | n/r |  | 202 | +15.0% | 174 | +11.7% | 210 | +16.0% |

   *Four indexes vs HSBC = 100; req = required outperformance, (premium)^(1/5) − 1 per year over a 5-year convergence horizon. Px = local per-share price, all dates 2026 — the staleness marker; peer per-share prices show the market-cap as-of date until the next fetch-peers run adds `SharePrice`. Caveats per Table 6: AUM-definition differences drive much of the P/Cap spread; CBA's wealth AUM is undisclosed (`n/r` on that lens). Sources: Table 6; Table 4; 1Q2026 valuation table.*
...
*Scope: questions from `guides/frame.md` (6 questions, formats per its internal notes, 2026-07-24); grounded in report.md (incl. Table 6) + data/signals.md (v2026.07.24-r3). Closed-book run by Claude (CwClFable5) — the SOP-recommended non-Claude rerun remains advisable. Not investment advice.*
<!-- conclusions:end -->
...
Monetization (Frame Q5) — indexed to HSBC = 100 (latest full FY per bank; both ratios are computed within one bank, so reporting currencies cancel). vDep = Monetization_vDeposits · vCap = Monetization_vCapitalBase (definitions below):
...
Relative valuation (Frame Q6) — four indexes vs HSBC = 100; req %/yr = required outperformance, (premium ratio)^(1/5) − 1 per year (5-yr convergence). Px = local per-share price with its as-of date — the staleness marker (P/Cap = P/CapitalBase):
...
*Definitions: Monetization_vDeposits = total revenue ÷ customer deposits · Monetization_vCapitalBase = total revenue ÷ (customer deposits + wealth AUM) · P/CapitalBase = market cap ÷ (deposits + AUM) · P/Rev = market cap ÷ total revenue · P/E = market cap ÷ net profit · P/B = market cap ÷ book equity. SG-bank fundamentals from the reconciled ledger (market cap = the Px column's dated price × FY25 shares); peer fundamentals from Tier-1 FY2025 filings (CBA FY to Jun-2025) with market caps dated Jul 2026, fetched 2026-07-24 by an independent retriever (Perplexity harness, stamp PxClOpus4.8) — `pipeline/sg-banks/data/peers.csv`. Peer per-share prices show `n/r` until the next fetch-peers run (a `SharePrice` metric was added to the SOP); the market-cap as-of date carries the staleness signal meanwhile. Caveats: AUM definitions differ per bank (largest driver of the vCap and P/Cap spread — e.g. UBS invested assets US$4.75tn dwarf its deposits); Commonwealth Bank discloses no comparable wealth AUM (`n/r` where it is required); China Merchants Bank market cap is A-share-basis; Standard Chartered market cap converted GBP→USD to match its USD reporting. Tables generated deterministically by `method/code/build_benchmarks.py` from `ledger.csv` + `peers.csv` (CI-verified).*
