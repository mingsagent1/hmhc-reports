...
| **Position** | `Frame → (Retrieve ‖ Scan) → Reconcile → Tables → …`. Build-Tables consumes only reconciled rows; it never re-opens retrieval. |
...
- **`single-px` / `single-cl`** — only one agent filled it → take it; the status names which. Cross-model confidence rules: same-model agreement can share a blind spot — see the provenance-stamp rationale in `update-ledger.md` §1.
...
Prefer **restated** figures (trap B6 in `update-ledger.md`) and record the original in the note. Every `resolved` row becomes a line in the report's Validation Report (Appendix A), lifted downstream via `tables.md`.
...
The reconciled `data/ledger.csv` feeds **Build-Tables** (`method/build-tables.md`). Reconcile publishes nothing to `reports/`.
