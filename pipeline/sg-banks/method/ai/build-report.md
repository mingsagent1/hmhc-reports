...
1. **Purpose** — opens the document. Contains: the **Thesis** (a succinct rendering of `guides/frame.md`'s thesis — condense wording, never change substance) · the **key questions** as a numbered list (succinctly reworded from `guides/frame.md`, same order and numbering, no questions invented or dropped) with a note that each is answered in the Conclusions · a one-line **Scope** (banks/tickers · period · currency · Appendix D pointer) · and the fixed disclaimer blockquote: *"**This is not financial advice.** It is a demonstration of the use of AI in business analysis."* There is no separate Interpretation section — Purpose replaced it (2026-07-22).
