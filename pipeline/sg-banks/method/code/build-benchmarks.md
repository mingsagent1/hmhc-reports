...
| **Sole output** | `data/benchmarks.md` — monetization indices (incl. the implied SG Other-Revenue uplift line) and four valuation indexes vs the index bank = 100 (Frame Q5/Q6; client assets = customer deposits + wealth AUM, renamed from Capital Base 2026-07-25), required 5-yr outperformance = (premium)^(1/5) − 1, and the wealth-hub flows table (Frame Q2). Table shapes per the frame's Q5/Q6 format notes: the Q5 table carries a **Top Other-Revenue** text column (from `peers.csv` `TopOtherRevenue`, `n/d` if absent); the Q6 table opens with a **Px (as-of)** column — SG banks: ledger `PriceCurrent` per-share price + its dated comment; peers: `SharePrice` from `peers.csv` when fetched, else `n/r (mcap <as-of>)` so staleness stays visible. Sections needing unfetched inputs are marked pending, with SG-only raw values shown meanwhile. |
